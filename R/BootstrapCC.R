#' BootstrapCC
#'
#' A fast Bootstrap implementation of consensus clustering plus algorithm
#'
...
#' @param maxK (numeric) maximum number of clusters to test (default 10)
#' @param reps  (numeric) maximum number of subsamplings iterations (default 100)
#' @param clusterAlg character value. cluster algorithm.  'hc' heirarchical (hclust), 'pam' for paritioning around medoids, 'km' for k-means upon data matrix, 'kmdist' for k-means upon distance matrices (former km option), or a function that returns a clustering.  See example and vignette for more details.
#' @param title character value for output directory. Directory is created only if plot is not NULL or writeTable is TRUE. This title can be an abosulte or relative path.
#' @param innerLinkage heirarchical linkage method for subsampling.
#' @param finalLinkage eirarchical linkage method for consensus matrix.
...
BootstrapCC <- function( d=NULL,
  ...
  if(.os == "Windows"){
    BPPARAM <- SnowParam(workers = ifelse( nCores > .available.cores, .available.cores -1, nCores),
                         type = "SOCK")
  }else{
    BPPARAM <-  MulticoreParam(ifelse( nCores > .available.cores, .available.cores -1, nCores))
  }
...
##------------------------
  if(is.null(ml)==TRUE){
    ...
    ml <- ccRun( d=d,
                 ...
                 distance=distance,
                 verbose=verbose,
                 corUse=corUse, BPPARAM=BPPARAM, bt = bt)
  }
  res=list();
...
ccRun <- function( d=d,
                   ...
                   weightsFeature=NULL,
                   verbose=NULL,
                   corUse=NULL, BPPARAM=bpparam(), bt = FALSE) {
...
paralelo<-bplapply(1:repCount, function(repet){
    #############################################################################################
    if(verbose){
      message(paste("random subsample", repet));
    }
    ## take expression matrix sample, samples and genes
    sample_x = sampleCols( d, pItem, pFeature, weightsItem, weightsFeature, bt )
    ...
    listaK<-lapply(2:maxK, function(k){
    ...
    })#Termina el Bucle de K

    return(list(K=listaK,samplex3=sample_x[[3]]))
  },BPPARAM = BPPARAM)

  ###########POST PROCESAMIENTO#########
  #browser()
  for(i in 1:repCount){
    mCount <- connectivityMatrix(rep(1, length(paralelo[[i]]$samplex3)),
                                 mCount,
                                 paralelo[[i]]$samplex3)
    for(j in 2:maxK){
      ml[[j]] <- connectivityMatrix( paralelo[[i]]$K[[j-1]], ml[[j]], paralelo[[i]]$samplex3)
    }
  }
  #browser()
...
sampleCols <- function( d,
  ...
  #Si bt = FALSE, HACE LO QUE VENIA HACIENDO
  ...
  sampCols<- sort(unique(sample(space, space , replace = T)))
  }
  ...
  if ( inherits( d, "matrix" ) ) {
    if ( (! is.null( pRow ) ) &&
      ...
      ## only sample the rows and generate a sub-matrix if we're sampling over the row/gene/features
       ...
       #Si BT , false, sigue haciendo esto.
     ...
     {sampRows<- sort(unique(sample( space, space, replace = T)))}
